...
def prepare_ga_inputs(
    ...
    customer_list = get_customer_list(
        locations=locations, ignored_customers=ignored_customers, completed_customers=completed_customers
    )
    ...
    if pm == "TDVRP":
        ...
        if not multithreaded:
            ...
            output = genetic_algorithm_vrp(
                ...
                population_count=random_perm_count,
                iteration_count=iteration_count,
            )
        ...
        output_dict = result_2_output.vrp_result_2_output(
            vehicles_start_times=ist,
            duration=duration,
            load=load,
            locations=locations,
            vrp_result={"vehicles_routes": output[2]},
            capacities=capacities,
        )
        ...
        output_dict = result_2_output.tsp_result_2_output(
            start_time=initial_start_times,
            start_node=sn,
            duration=duration,
            locations=locations,
            tsp_result={"route": output[2][0]},
            load=load,
