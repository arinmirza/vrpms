...
def genetic_algorithm(
    population, N, M, k, q, W, duration, demand, ist, demand_dict, sn, cancelled_customers, do_load_unload
):
    """
...
def calculate_duration_load_unload_tsp(
    ...
    if do_loading_unloading:
        if current_location != DEPOT:
            current_time += (
                UNLOADING_CUSTOMER_TIME_INIT + UNLOADING_CUSTOMER_TIME_PER_UNIT * demand_dict[current_location]
            )
        else:
            total_load = 0
...
def calculate_duration(permutation, VST, dist_data, M, Q, load, demand_dict, sn, cancelled_customers, do_load_unload):
    ...
    route_max_time, route, vehicle_routes, vehicle_times = calculate_duration_load_unload_tsp(
        current_time=vehicles_start_times,
        current_location=sn,
        perm=permutation,
        duration=dist_data,
        load=load,
